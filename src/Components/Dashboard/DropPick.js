...
const DropPick = ({ boarding, dropping, bus_type_status }) => {
        ...
        >
          <div className="grid grid-cols-2 ">
            <div className="flex flex-col  justify-start px-[1vw]">
              <p className="text-[1.5vw] text-[#1F487C]  font-extrabold py-[1vw]">
                BOARDING
              </p>
              ...
              <div className="grid grid-cols-2">
                {boarding.map((item) => (
                  <div className="flex gap-x-[1vw]  pb-[.7vw]">
                    <div className="text-[1vw] text-[#1F487C] font-bold">
                      {`${dayjs(item.time)?.format("HH:mm")}`}{" "}
                    </div>
                    {item?.name?.length > 15 ? (
                      <Popover content={item?.name}>
                        {" "}
                        <div className="text-[1.1vw] cursor-pointer text-[#1F487C]">
        ...
        <div className="bg-[#F6F6F6] h-auto w-full px-[5vw] py-[1vw]">
          <div className="grid grid-cols-2">
            <div className="flex flex-col gap-y-[2vw]">
              ...
              {boarding.map((item) => (
                <div className="flex gap-[1vw]">
                  <div className="text-[2.4vw] text-[#1F487C] font-bold">{`${dayjs(
                    item.time
                  ).format("HH:mm")}`}</div>
            ...
            <div className="flex flex-col gap-y-[2vw]">
              ...
              {dropping.map((item) => (
                <div className="flex gap-[1vw]">
                  <div className="text-[2.4vw] text-[#1F487C] font-bold">
                    {`${dayjs(item.time).format("HH:mm")}`}
