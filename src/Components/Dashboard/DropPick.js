    ...
    return (
        <>
            <div className='md:block hidden w-full'>
                <div className='bg-[#F6F6F6] h-auto w-full px-[1vw]'>
                    <div className='grid grid-cols-2 '>
                        <div className='flex flex-col  justify-start px-[1vw]'>
                            <p className='text-[1.5vw] text-[#1F487C]  font-extrabold py-[1vw]'>BOARDING</p>
                            ...
                            <div className='grid grid-cols-2'>
                                {boarding.map((item) => (
                                    <div className='flex gap-x-[1vw]  pb-[.7vw]'>
                                        <div className='text-[1vw] text-[#1F487C] font-bold'>{`${dayjs(item.time)?.format('HH:mm')}`} </div>
                                      {item?.name?.length > 15 ?<Popover content={item?.name}> <div className='text-[1.1vw] cursor-pointer text-[#1F487C]'>{item?.name?.substr(0,15)}...</div></Popover> : <div className='text-[1.1vw] text-[#1F487C]'>{item.name}</div> }  
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className='flex flex-col justify-start px-[1vw]'>
                            <p className='text-[1.5vw] text-[#1F487C]  font-extrabold py-[1vw]'>DROPPING</p>
                            {/* <div className='w-[30vw] h-[20vw] overflow-x-auto'>
                            ...
                            <div className='grid grid-cols-2'>
                                {dropping.map((item) => (
                                    <div className='flex gap-x-[1vw]  pb-[.7vw]'>
                                        <div className='text-[1vw] text-[#1F487C] font-bold'>{`${dayjs(item.time).format('HH:mm')}`}<span className='text-gray-400 ps-[.5vw] font-[.2vw]'>{`(${dayjs(item.time).format('DD')} ${"-"} ${dayjs(item.time).format('MMM')})`}</span> </div>
                                        {item?.name?.length > 15 ?<Popover content={item?.name}> <div className='text-[1.1vw] cursor-pointer text-[#1F487C]'>{item?.name?.substr(0,15)}...</div></Popover> : <div className='text-[1.1vw] text-[#1F487C]'>{item.name}</div> }  
                                    </div>
                                ))}
                            </div>
                ...
                <div className='bg-[#F6F6F6] h-auto w-full px-[5vw] py-[1vw]'>
                    <div className='grid grid-cols-2'>
                        ...
                        <div className='flex flex-col gap-y-[2vw]'>
                            ...
                            {dropping.map((item) => (
                                <div className='flex gap-[1vw]'>
                                    <div className='text-[2.4vw] text-[#1F487C] font-bold'>{`${dayjs(item.time).format('HH:mm')}`}<span className='text-gray-400 ps-[.5vw]'>{`(${dayjs(item.time).format('DD')} ${"-"} ${dayjs(item.time).format('MMM')})`}</span></div>
                                    <div className='text-[2.4vw] text-[#1F487C]'>{item.name}</div>
                                </div>
                            ))}
                        </div>
